...
from data_loader.skeleton_dataset import Skeleton_Data
...
class SkeletonDataLoader(BaseDataLoader):
    
    def __init__(self, data_dir, list_path, batch_size, shuffle, validation_split, num_workers, training=True):
        # trsfm = transforms.Compose([ToTensor(),
        #                             ])
        trsfm = None
        self.data_dir = data_dir
        self.list_path = list_path
        self.dataset = Skeleton_Data(self.data_dir, list_path=self.list_path, train=training, transform=trsfm)
        super(SkeletonDataLoader, self).__init__(self.dataset,
